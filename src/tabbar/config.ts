...
export const customTabbarList: CustomTabBarItem[] = [
  {
    text: '首页',
    pagePath: 'pages/index/index' as any,
    iconType: 'unocss',
    icon: 'i-carbon-home',
  },
  {
    text: '我的',
    pagePath: 'pages/me/me' as any,
    iconType: 'unocss',
    icon: 'i-carbon-user',
  },
]

// 生成用于 pages.json 的 tabbar 列表，需包含全部可能的 tabbar 页面，否则 switchTab 会报错
function buildMergedTabbarList() {
...
const _tabbarList = customTabbarEnable
  ? buildMergedTabbarList().map(item => ({ text: item.text, pagePath: item.pagePath }))
  : nativeTabbarList
export const tabbarList = customTabbarEnable ? buildMergedTabbarList() : nativeTabbarList
